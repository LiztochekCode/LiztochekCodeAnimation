...
@app.on_message(filters.command("lera", prefixes=".") & filters.me)
def betalove(_, msg):
    time = 0.5
    for i in range(1):
        ...
        sleep(time)
        msg.edit(f"<b>Лерочка самая лучшая!!! </b>")
...
def valentine(_, msg):
    ...
    sleep(time)
    msg.edit(f'<b> 💖 Лерочка самая лучшаяяя 💖 </b>')
    sleep(time)
...
async def valentine(client, message):
    ...
    await message.edit("Сделайте озвучку плз @Liztochekkkk")
